...
var ccToClient = map[string]string{"aa:bb:cc:dd:ee:ff": "00:11:22:33:44:55"}
var clientToCc = map[string]string{"00:11:22:33:44:55": "aa:bb:cc:dd:ee:ff"}

func TestLoadCcClientTable(t *testing.T) {
	computedCcToClient, computedClientToCc := LoadCcClientMappings("states_test.json")

	if !reflect.DeepEqual(computedCcToClient, ccToClient) {
		t.Error("Error in LoadCcClientMappings(): unexpected mapping returned")
	}
	if !reflect.DeepEqual(computedClientToCc, clientToCc) {
		t.Error("Error in LoadCcClientMappings(): unexpected mapping returned")
	}
}
